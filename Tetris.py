...
BOARDHEIGHT = 25
BLANK = '.'
XMARGIN = int((WINDOWWIDTH - BOARDWIDTH * BOXSIZE) / 2 - 40)
...
def run_game():
    ...
    while True:
        ...
        if moving_down and time.time() - last_movedown_time > MOVEDOWNFREQ and is_valid_position(board, falling_piece,
                                                                                                 adj_Y=1):
            falling_piece['y'] += 1
            last_movedown_time = time.time()
...
def draw_status(score, level, Generation=None, max_score=None, flag=False):
    """Draw status"""
    ...
    if flag:
        genSurf = BASICFONT.render('Generation: #%s' % Generation, True, TEXTCOLOR)
        genRect = genSurf.get_rect()
        genRect.midright = (WINDOWWIDTH - 20, 350)
        DISPLAYSURF.blit(genSurf, genRect)

        scoreSurf = BASICFONT.render('Max score: %s' % max_score, True, TEXTCOLOR)
        scoreRect = scoreSurf.get_rect()
        scoreRect.midright = (WINDOWWIDTH - 20, 390)
...
def calc_move_info(board, piece, x, r, total_holes_bef, total_blocking_bloks_bef):
    ...
    return [True, num_removed_lines, new_holes, lowest_clearable_line(board), new_blocking_blocks, piece_sides,
            floor_sides, wall_sides, weighted_blocks_log(board)]
    ...
    for Px in range(TEMPLATEWIDTH):
        for Py in range(TEMPLATEHEIGHT):
            ...
            if not PIECES[piece['shape']][piece['rotation']][Py][Px] == BLANK:  # Quadrante faz parte da peça
                ...
                if Px == TEMPLATEWIDTH - 1 and piece['x'] + Px < BOARDWIDTH - 1 and not board[piece['x'] + Px + 1][
                                                                                            piece['y'] + Py] == BLANK:
                    piece_sides += 1

            # Other pieces in general
            elif piece['x'] + Px < BOARDWIDTH and piece['x'] + Px >= 0 and piece['y'] + Py < BOARDHEIGHT and not \
                    board[piece['x'] + Px][
                        piece['y'] + Py] == BLANK:  # quadrante do tabuleiro colorido mas nao do template
...
def lowest_clearable_line(board):
    '''
    This function is designed to find the first line (row) on a game board that can be cleared.

    :param board: current game board.
    :return: the first clearable line if found else it returns -1.
    '''
    return next((row for row in reversed(range(len(board))) if all(cell for cell in board[row])), -1)


def weighted_blocks_log(board):
    '''
    This function calculates a weighted sum of the logarithms of the row indices for all filled cells in the game board.

    :param board: current game board.
    :return: The weighted sum of the logarithms of the row indices for all filled cells.
    '''
    return sum(math.log1p(row) * (len(board) - row) for row in range(len(board)) for col in range(len(board[0])) if
               board[row][col])


def calc_best_move(board, piece, chromosome):
    '''
    This function calc the best coordinates (x , y) and best rotation for this piece on this board.

    :param board: current game board.
    :param piece: current piece to calculate its coordinates.
    :param chromosome: the weights of the agent to decide which move is the best.
    '''
    ...
    total_holes_bef, total_blocking_blocks_before = calc_initial_move_info(board)

    for r in range(len(PIECES[piece['shape']])):
        for x in range(-2, BOARDWIDTH - 2):
            play = calc_move_info(board, piece, x, r, total_holes_bef, total_blocking_blocks_before)
            if play[0]:
                score = 0
                for i in range(len(chromosome)):
                    score += (chromosome[i] * play[i + 1])
                if score > best_score:
                    best_y = piece['y']
                    best_r = r
                    best_x = x
...
def AI_game_mode(max_score=200000, test=False, chromosome=None, gen=None, best_score=None):
    '''
    This game manages Tetris game for AI agents to play.

    :param max_score: highest score that the game ends after.
    :param test: if this game is for a test analysis or not.
    :param chromosome: the weights of the agents to use them.
    :param gen: the number of generation that the agent from.
    :param best_score: the best score that has been found so far.
    :return: the score this agent.
    '''
    global FPSCLOCK, DISPLAYSURF, BASICFONT, BIGFONT
    pygame.init()
    ...
    while True:
        ...
        if test:
            ...
            mx = max(best_score, score)
            draw_status(score, level, gen, mx, True)
